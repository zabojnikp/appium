...
describe('Feedbacks', () => {
    ...
    it('should send a feedback', async () => {
        ...
        await browser.pause(1000); //dont use pause, use waitFor functions
        await $(feedbackMenuItem).click(); 
        await expect($(FeedbackTextName)).toHaveTextContaining('Váš názor nás zajímá');//check for visibility
        ...
        await expect($(FeedbackConfirmPage)).toHaveTextContaining('DĚKUJEME');//check for visibility
        await expect($(SkipStepButton)).toHaveTextContaining('Přeskočit tento krok');
        await $(SkipStepButton).click();
        // check user is redirected , page not visible, menu page visible
    });
});